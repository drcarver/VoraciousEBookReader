...
namespace SimpleEPUBReader
{
    ...
    public sealed partial class MainPage : Page, Navigator.ISetAppColors, SimpleBookHandler, ISetImages
    {
        ...
        private async void MainPage_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (App.SavedActivatedBooks != null)
            {
                var nav = Navigator.Get();
                nav.DisplayBook(Navigator.NavigateControlId.InitializationCode, App.SavedActivatedBooks, App.SavedActivatedBookLocation);
            }
            else if (id != null)
            {
        ...
        private static async Task<Books> InsertFileIntoDatabase(BooksContext bookdb, IFolder folder, string filename, bool getFullData = false)
        {
            Books Books = null;
            if (bookdb == null) bookdb = BooksContext.Get();
            ...
            try
            {
                ...
                {
                    Books = CommonQueries.BookGet(bookdb, wd.BookId);
                    ...
                    // then the Books here is non-null, but also not really filled in well.
                    if (Books == null || Books.BookSource.StartsWith (Books.BookSourceBookMarkFile))
                    {
                        if (wd.BD != null)
                        {
                            ...
                            if (!string.IsNullOrEmpty(wd.BD.BookId))
                            {
                                ...
                                wd.BD.BookSource = Books.BookSourceUser;

                                // Add in possible data from the Books set by the user
                                if (Books != null)
                                {
                                    wd.BD.NavigationData = Books.NavigationData;
                                    wd.BD.Review = Books.Review;
                                    wd.BD.Notes = Books.Notes;
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"{filename}({wd.BookId}) is {Books.GetBestTitleForFilename()}");
                    }
        ...
        /// <returns></returns>
        public static async Task MarkAllDownloadedFiles(BooksContext bookdb, IFolder folder, bool getFullData = false)
        {
        ...
        {
            BooksContext bookdb = BooksContext.Get();
        ...
        private async Task DoOpenFilesAsync(BooksContext bookdb, IReadOnlyList<IStorageItem> items)
        {
            ...
            foreach (var item in items)
            {
                ...
                if (storageFile.Name.ToLower().EndsWith(".epub"))
                {
                    ...
                    {
                        var Books = await InsertFileIntoDatabase(bookdb, downloadFolderFolder, storageFile.Name, true);
                        if (Books != null && isFirst)
                        {
                            // Only display the first book
                            var nav = Navigator.Get();
                            nav.DisplayBook(Navigator.NavigateControlId.DroppedFile, Books);
        ...
        {
            BooksContext.ResetSingleton("InitialBooks.Db");
            var bookdb = BooksContext.Get();
            ...
            ;
            bookdb.SaveChanges();
            BooksContext.ResetSingleton(null);
        }
...
// changes are much more painful than anticipated.
        ...
        private async void OnCreateDatabase(object sender, RoutedEventArgs e)
        {
            ...
            BooksContext.ResetSingleton(null); // reset database
        }
        ...
        public Task DisplayBook(Books book, BookLocation location)
        {
