...
namespace SimpleEPUBReader.Controls
{
    ...
    public sealed partial class BookSearch : UserControl, SimpleBookHandler
    {
        ...
        {
            var bookdb = BooksContext.Get();
            var book = (sender as BookCard)?.DataContext as Books;
            if (book == null) return;
        ...
        public async Task DisplayBook(Books book, BookLocation location)
        {
            await Task.Delay(0);
            ReloadAllNotes();
        ...
        {
            var item = args.SwipeControl.DataContext as Books;
        ...
        private BookCard GetBookCardFromBooks(Books Books)
        {
            if (Books == null) return null;
            var container = uiBookList.ContainerFromItem(Books) as ListViewItem;
            if (container == null) return null;
            var swipe = container.ContentTemplateRoot as SwipeControl;
            if (swipe == null) return null;
            var bookcard = swipe.Content as BookCard;
        ...
        public static async Task DoSwipeDownloadOrReadAsync(Books Books)
        {
            if (BookSearchSingleton == null) return;
            var bookcard = BookSearchSingleton.GetBookCardFromBooks(Books);
            if (bookcard != null)
            {
                bookcard.ResetDownloadPanel();
                await BookSearchSingleton.DoSwipeDownload(Books);
            }
        }
        private async void OnSwipeDownload(SwipeItem sender, SwipeItemInvokedEventArgs args)
        {
            ...
            var Books = bookcard.GetBooks();
            await DoSwipeDownload(Books);
        }



        private async Task DoSwipeDownload(Books Books)
        {
            var bookcard = GetBookCardFromBooks(Books);

            var bookdb = BooksContext.Get();
            var nd = CommonQueries.BookNavigationDataEnsure(bookdb, Books);
            nd.NSwipeRight++;
            ...
            SetupDownloadsIfNeeded(Books);

            // But wait! If the book is already downloaded, then just display it
            var fileStatus = Books.DownloadData == null ? DownloadData.FileStatus.Unknown : Books.DownloadData.CurrFileStatus;
            switch (fileStatus)
            {
                case DownloadData.FileStatus.Downloaded:
                    var nav = Navigator.Get();
                    nav.DisplayBook(ControlId, Books);
                    break;
                default:
        ...
        private void OnSwipeRemove(SwipeItem sender, SwipeItemInvokedEventArgs args)
        {
            ...
            Books = args.SwipeControl.DataContext as Books;
            Books.Remove(Books);
        }

        private void SetupDownloadsIfNeeded(Books Books)
        {
            if (Books != null)
            {
                var containerObject = uiBookList.ContainerFromItem(Books);
        ...
        {
            var Books = e.RemovedItems.Count == 1 ? e.RemovedItems[0] as Books : null;
            if (Books != null)
            {
                var containerObject = uiBookList.ContainerFromItem(Books);
            ...
            Books = e.AddedItems.Count == 1 ? e.AddedItems[0] as Books : null;
            SetupDownloadsIfNeeded(Books);
        }
