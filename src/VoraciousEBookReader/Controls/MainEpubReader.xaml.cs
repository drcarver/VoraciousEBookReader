...
namespace SimpleEPUBReader.Controls
{
    ...
    public sealed partial class MainEpubReader : UserControl, INavigateTo, BookHandler, ISetAppColors
    {
        ...
        private Books Books { get; set; } = null;
        ...
        /// <returns></returns>
        public async Task DisplayBook(Books Books, BookLocation location = null)
        {
            ...
            var dd = Books.DownloadData ?? CommonQueries.DownloadedBookFind(bookdb, Books.BookId);
            var nav = Books.NavigationData ?? CommonQueries.BookNavigationDataFind(bookdb, Books.BookId);
            ...
            switch (openResult)
            {
                case OpenResult.OK:
                    ...
                    appView.Title = Books.Title.Replace("\n", " -- "); // title is multi-line
                    break;
                ...
                case OpenResult.RedownloadableError: // An error. Mark the book as not downloaded + redownload
                    ...
                    await BookSearch.DoSwipeDownloadOrReadAsync(Books);
                    break;
        ...
        {
            var bookdb = BooksContext.Get();
            var review = CommonQueries.UserReviewFind(bookdb, Books.BookId);
        ...
        private async Task<OpenResult> OpenFile(string fullFilePath, BookLocation location)
        {
            ...
            try
            {
                ...
                if (fileContents == null)
                {
                    ...
                    var md = new MessageDialog($"Error: book file is missing: {Books.Title} ")
                    {
        ...
        public void NavigateTo(NavigateControlId sourceId, BookLocation location)
        {
            ...
            var navigationData = CommonQueries.BookNavigationDataFind(bookdb, Books.BookId);
            if (navigationData == null)
            {
                navigationData = new BookNavigationData() { BookId = Books.BookId, CurrStatus = BookNavigationData.UserStatus.Reading };
...
";
        ...
        private async void OnReviewClicked(object sender, RoutedEventArgs e)
        {
            ...
            review.SetBooks(Books, defaultReviewText);
        ...
        private BookNavigationData EnsureBookNavigationData(BooksContext bookdb)
        {
        ...
        private async Task SavePositionNow()
        {
            ...
            localSettings.Values[CURR_READING_BOOK_ID] = Books.BookId;
