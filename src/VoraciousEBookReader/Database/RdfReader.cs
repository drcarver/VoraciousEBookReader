...
namespace SimpleEPUBReader.Database
{
    static class RdfReader
    {
        ...
        /// <returns></returns>
        public static async Task<int> ReadZipTarRdfFile(BooksContext bookdb)
        {
        ...
        public static async Task<int> ReadZipTarRdfFileAsync(IndexReader ui, BooksContext bookdb, Windows.Storage.StorageFile file, CancellationToken token, UpdateType updateType = UpdateType.Full)
        {
                                    ...
                                    using (var tarReader = ReaderFactory.Open(tarStream))
                                    {
                                        while (tarReader.MoveToNextEntry())
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if (newCount > 0)
                                                {
                                                    foreach (var Books in newBooks)
                                                    {
                                                        await ui.OnAddNewBook(Books);
                                                    }
                                                }
        ...
        /// </summary>
        /// <returns></returns>
        public static async Task<int> ReadDirAsync(BooksContext bookdb)
        {
...
#if NEVER_EVER_DEFINED
        private static async Task ReadDirAsyncFolder(BooksContext bookdb, StorageFolder folder)
        {
            uint startIndex = 0;
            uint maxItems = 1000;
    ...
    <marcrel:trl>
      <pgterms:agent rdf:about="2009/agents/19">
        ...
        private static Books ExtractBook(string logname, XmlNode node)
        {
            var book = new Books();
            ...
            foreach (var cn in node.ChildNodes)
            {
                ...
                switch (value.Name)
                {
                    ...
                    case "dcterms:type":
                        ...
                        switch (bookType)
                        {
                            case "Collection": book.BookType = Books.FileType.Collection; break;
                            case "Dataset": book.BookType = Books.FileType.Dataset; break;
                            // Dataset from e.g. http://www.gutenberg.org/ebooks/3503
                            // These are e.g. the human genome project. They are 100% uninteresting.
                            case "Image": book.BookType = Books.FileType.Image; break;
                            case "MovingImage": book.BookType = Books.FileType.MovingImage; break;
                            case "Sound": book.BookType = Books.FileType.Sound; break;
                            case "StillImage": book.BookType = Books.FileType.StillImage; break;
        ...
        private static Books[] BooksArray = new Books[10];
        private static int BooksArrayIndex = -1;
        ...
        /// <returns></returns>
        private static int ReadRdfFileAndInsert(BooksContext bookdb, string logname, string xmlData, IList<Books> newBooks, UpdateType updateType)
        {
                            ...
                            switch (bookNode.Name)
                            {
                                case "pgterms:ebook":
                                    ...
                                    if (book != null && book.Validate() == "") //"" is OK (yes, it's weird)
                                    {
                                        ...
                                        BooksArrayIndex = (BooksArrayIndex + 1) % BooksArray.Length;
                                        BooksArray[BooksArrayIndex] = book;
