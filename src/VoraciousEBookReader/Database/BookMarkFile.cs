...
namespace SimpleEPUBReader.Database
{
    ...
    public class BookMarkFile
    {
        ...
        public List<Books> BookMarkList { get; set; }
        ...
        private static async Task<BookMarkFile> ReadFileAsBookMarkFileAsync(StorageFolder folder, string bookmarkfilename)
        {
            ...
            if (bmf == null)
            {
                ...
                try
                {
                    var list = JsonConvert.DeserializeObject<List<Books>>(str);
                    if (list != null)
                    {
        ...
        private static async Task<int> MergeAsync(BookMarkFile bmf)
        {
            ...
            foreach (var external in bmf.BookMarkList)
            {
                ...
                if (book == null)
                {
                    ...
                    if (!external.BookSource.StartsWith (Books.BookSourceBookMarkFile))
                    {
                        external.BookSource = Books.BookSourceBookMarkFile + external.BookSource;
                    }
        ...
        public static BookMarkFile CreateBookMarkFile(BookMarkFileType fileType)
        {
            ...
            // We only save some of the Books fields in a book mark file. 
            // Don't bother with the full file list (total waste of time), or the people list.
            var trimmedList = new List<Books>();
            foreach (var book in list)
            {
                trimmedList.Add(CreateBookMarkBooks(book));
            }
        ...
        /// <summary>
        /// A full Books has too much data to write into the BookMark file. Trim it down
        ...
        /// <returns></returns>
        private static Books CreateBookMarkBooks(Books source)
        {
            ...
            // DownloadData, Files and People are explicitly kept blank
            return retval;
        }
