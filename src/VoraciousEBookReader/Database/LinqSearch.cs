...
using VoraciousEBookReader.EPUB.Searching;
...
namespace SimpleEPUBReader.Searching
{
    static public class AllBookSearch
    {
        ...
        /// <returns></returns>
        static public List<Books> SearchInternal(BooksContext bookdb, string searchScope, string search, string language, string sortBy, out bool andMore)
        {
            ...
            IQueryable<Books> queryList = bookdb.Books;
            ...
            IEnumerable<Books> enumerableList = null;

            // Final list of books to return.
            var resultList = new List<Books>();
            ...
            switch (searchScope)
            {
                ...
                case "CopiedToEBookReader":
                    matchList = queryList
                        .Where(b => b.NavigationData != null)
                        .Where(b => b.NavigationData.NSwipeLeft < 1)
                        .Where(b => b.NavigationData.CurrStatus == Navigation.UserStatus.CopiedToEBookReader)
                        ;
                    break;
            }
            // Add in sorting
            ...
            else
            {
                ...
                foreach (var book in matchList)
                {
                    var epubMatch = true; // turn this off until it works !mustIncludeEpub || Books.FilesIncludesEpub(book);
