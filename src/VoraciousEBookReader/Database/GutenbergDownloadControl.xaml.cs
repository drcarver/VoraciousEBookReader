...
namespace SimpleEPUBReader.Database
{
    ...
    {
        void BookEnd(BookStatus status, Books book);
    ...
    public sealed partial class GutenbergDownloadControl : UserControl, IndexReader
    {
        ...
        { 
            ...
            if (gotfile)
            {
                ...
                await Task.Run(async () =>
                {
                    ...
                    BooksContext.ResetSingleton(null); // must reset database (otherwise no records can be found)
                    ;
                });
                ;
            }
        ...
        public async Task OnAddNewBook(Books Books)
        {
            NNewBooks++;
            await this.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () => {
                var title = Books.Title.Replace("\n", " ");
                uiAdding.Text = $"Adding or updating {NNewBooks}: {title}";
                uiLog.Text += $"{Books.Title}\n";
            });
        }
