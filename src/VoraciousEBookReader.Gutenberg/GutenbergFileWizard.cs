namespace VoraciousEBookReader.Gutenberg
{
    ...
    public static class GutenbergFileWizard
    {
        public static int GetBookIdNumber(string bookId, int defaultValue)
        {
            ...
            int id;
            bool ok = int.TryParse(rest, out id);
            if (!ok) return defaultValue;
            return id;
        }

        public static async Task<WizardData> GetDataAsync(WizardData data, bool getBooks = false)
        {
            ...
            if (data.BD != null)
            {
                data.BD.BookType = Books.FileType.Text;
                data.BD.Title = title;
                if (data.BD.People == null) data.BD.People = new ObservableCollection<Person>();
                foreach (var author in book.Authors)
                {
                    data.BD.People.Add(new Person(author, Relator.author));
                }
            }
                    ...
                    switch (child.NodeName)
                    {
                        ...
                        case "dc:publisher":
                            if (data.BD != null)
                            {
                                ...
                                data.BD.People.Add(new Person(text, Relator.publisher));
                            }
                            break;
                    }
                }
        ...
        private string FixupLanguage(string input)
        {
            input = input.ToLower();
